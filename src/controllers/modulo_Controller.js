...
var Modulo = require('../models/modulo_Models');
...
async function getById(req, res){
	...
	const user = await Modulo.findByPk(id);
	//Manejo de errores
	if (user === null) {
		...
		console.log("Es instancia de modulo: " + user instanceof Modulo); // true
...
async function getAll(req, res){
	...
	console.log(modulos.every(modulo => modulo instanceof Modulo)); // true
	console.log("Todos los módulos:", JSON.stringify(modulos, null, 2)); //Muestra modulos por consola.
...
async function save(req, res){
	//Crea nuevo registro
	const mod = Modulo.build({ 
		//ID: No es necesario crearlo, el model se encarga de generar y verificar los duplicados.
        nombre: req.body.nombre,
        slug: req.body.slug,
        is_activo: req.body.is_activo,
        usuario_id: req.user_id
	});
	console.log("Es instancia de Usuario: " + mod instanceof Usuario); // true
	console.log("El ID generado para el usuario es: " + mod.id); //ID
	await mod.save(); //Guarda el registro.
	res.send({
